...
div()
    ...
    .bg(Colors::for_appearance(window).background)
    .child(div().child("左侧"))
    .child(div().child("右侧"))
```
...
div()
    ...
    .bg(Colors::for_appearance(window).background)
    .hover(|this| {
        this.bg(Colors::for_appearance(window).container)
            .cursor_pointer()
    })
    .child("悬停时变色")
```
...
div()
    ...
    .bg(Colors::for_appearance(window).background)
    .active(|this| {
        this.bg(Colors::for_appearance(window).selected)
    })
...
div()
    ...
    .border_color(Colors::for_appearance(window).border)
    .focus(|this| {
        this.border_color(Colors::for_appearance(window).selected)
    })
...
div()
    ...
    .when(selected, |this| {
        this.bg(Colors::for_appearance(window).selected)
            .text_color(Colors::for_appearance(window).selected_text)
    })
    .child(&item_text)
```
...
div()
    ...
    .when(is_active_task, |this| {
        this.border_2()
            .border_color(Colors::for_appearance(window).selected)
    })
    .child(&task_name)
```
...
div()
    ...
    .when_some(border_width, |this, w| {
        this.border_1().border_color(Colors::for_appearance(window).border)
    })
